...
/mob/camera/blob/process()
	if(!blob_core)
		...
		priority_announce("Биоопасность достигла критической массы. Потеря станции неизбежна.", "Оповещение о биологической опасности")
	...
	if(announcement_time && (world.time >= announcement_time || blobs_legit.len >= announcement_size) && !has_announced)
		priority_announce("Подтверждена вспышка биологической опасности 5-го уровня на борту [station_name()]. Весь персонал должен сдерживать угрозу.", "Оповещение о биологической опасности", ANNOUNCER_OUTBREAK5)
		has_announced = TRUE

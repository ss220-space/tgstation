...
/datum/round_event/sandstorm/announce(fake)
	...
	priority_announce("Большая волна космической пыли приближается со стороны [start_side_text] к станции. \
		Удар ожидается в ближайшие две минуты. Всем сотрудникам рекомендуется по возможности оказать помощь в ремонте и уменьшении ущерба.", "Collision Emergency Alert")

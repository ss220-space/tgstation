...
/datum/round_event/grey_tide/announce(fake)
	if(areasToOpen && areasToOpen.len > 0)
		priority_announce("Gr3y.T1d3 вирус обнаружен в [station_name()] подпрограммах для дверей. Уровень тяжести [severity]. Рекомендуется вовлечение ИИ станции.", "Оповещение о безопасности")
	else

...
/datum/round_event/shuttle_insurance/proc/answered()
	if(EMERGENCY_AT_LEAST_DOCKED)
		priority_announce("Друзья, вам определенно уже поздно приобретать страховку. Наши агенты не работают на месте.",sender_override = ship_name)
		return
	if(insurance_message && insurance_message.answered == 1)
		var/datum/bank_account/station_balance = SSeconomy.get_dep_account(ACCOUNT_CAR)
		if(!station_balance?.adjust_money(-insurance_evaluation))
			priority_announce("Вы не прислали нам достаточно денег на страховку шатла. Это, в терминах космического обывателя, считается мошенничеством. Мы оставим ваши деньги себе, мошенники!",sender_override = ship_name)
			return
		priority_announce("Благодарим вас за приобретение страховки для шатла!",sender_override = ship_name)
